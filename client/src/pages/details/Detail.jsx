...
const Detail = () => {
  ...
  useEffect(() =>{
    const getUsersMovie = async () =>{
      try {
        const res = await axios.get(`https://easymovie-api.herokuapp.com/api/user-list/${userId}/${movieId}`);
    ...
    const getMovie = async () =>{
      try {
        const res = await axios.get(`https://easymovie-api.herokuapp.com/api/movie/find/${movieId}`);
        setMovie(res.data)
        setIsLoading(false)
      } catch (error) {
        console.log(error);
  ...
  const addList = async () =>{
    try {
      await axios.post("https://easymovie-api.herokuapp.com/api/user-list/add", {userId, movieId});
      setAdded(!added);
    } catch (error) {
      console.log(error);
    }
  ...
  const deleteList = async () =>{
    try {
      await axios.delete(`https://easymovie-api.herokuapp.com/api/user-list/delete/${usersMovie?._id}`);
      setAdded(!added);
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <>
      <div className="banner" style={{backgroundImage: `url(${movie.movieBg})`}}>
      ...
      <div className="movie-content__poster">
        <div className="movie-content__poster__img" style={{backgroundImage: `url(${movie.moviePoster})`}}></div>
      </div>
      ...
      <div className="movie-content__info">
        ...
        <div className="btns">
            ...
            <OutlineButton className="small" onClick={added ? deleteList : addList}>{added ? `Remove from list` : `Add to list`}</OutlineButton>
        </div>
      </div>
    </div>}
